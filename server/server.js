...
app.get('/chartData', (req, res) => {
    //db.query("SELECT * FROM(SELECT * FROM iwp_sensor_data LEFT JOIN iwp_sensor_calculations ON iwp_sensor_data_id=iwp_sensor_data_id_fk WHERE iwp_pump_id_fk ='"+req.query.id+"' ORDER BY date_sensed DESC LIMIT 8) sub ORDER BY date_sensed ASC", (err, result) => {
    db.query("SELECT * FROM iwp_alt_data WHERE iwp_pump_access_fk = '"+req.query.id+"' ORDER BY date_sensed ASC LIMIT 8", (err, result) => {      
        if (err){
            console.log(err)
        } else {
            res.send(result)
...
app.get('/dangerData', (req, res) => {
    ...
    db.query("select * from iwp_alt_data order by date_sensed desc limit 17", (err, result) => {
        if (err){
            console.log(err)
        } else {
            res.send(result)
